...
function batchExecute(statements, label = 'batch') {
  ...
  for (let i = 0; i < statements.length; i += BATCH_SIZE) {
    ...
    if (res !== null) {
      ...
      console.log(`[D1] ${label} batch ${Math.floor(i / BATCH_SIZE) + 1}/${totalBatches}: ${chunk.length} statements`);
    } else {
      consecutiveFailures++;
      console.error(`[D1] ${label} batch ${Math.floor(i / BATCH_SIZE) + 1}/${totalBatches}: FAILED (consecutive: ${consecutiveFailures})`);
      if (consecutiveFailures >= 3) {
        console.error(`[D1] ABORT: 3 consecutive failures in "${label}". Stopping to preserve D1 quota.`);
        break;
      }
    }
  }
